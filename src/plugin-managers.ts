...
export const PLUGIN_MANAGERS: Record<string, PluginManager> = {
  "oh-my-zsh": {
    name: "oh-my-zsh",
    cacheCleanCommand: "rm -rf ~/.oh-my-zsh/cache ~/.oh-my-zsh/custom/plugins/* 2>/dev/null || true",
    configFiles: [
      { path: "~/.zshrc", template: "oh-my-zsh.zshrc" }
    ],
    generatePluginLoad: (plugin) => 
      `source ~/.oh-my-zsh/custom/plugins/${plugin.split('/')[1]}/${plugin.split('/')[1]}.plugin.zsh 2>/dev/null || true`,
    preInstallCommand: async (plugins: string[]) => {
      // Clone plugins for oh-my-zsh
      for (const plugin of plugins) {
        const pluginName = plugin.split('/')[1];
        await runCommand(`git clone https://github.com/${plugin}.git ~/.oh-my-zsh/custom/plugins/${pluginName} 2>/dev/null || true`);
      }
    },
  },
  ...
  "prezto": {
    name: "prezto",
    cacheCleanCommand: "rm -rf ~/.zprezto-contrib/* ~/.zprezto/cache ~/.cache/prezto 2>/dev/null || true",
    configFiles: [
    ...
    preInstallCommand: async (plugins: string[]) => {
      ...
      for (const plugin of plugins) {
        const pluginName = plugin.split('/')[1];
        await runCommand(`git clone https://github.com/${plugin}.git ~/.zprezto-contrib/${pluginName} 2>/dev/null || true`);
      }
    },
  },
  ...
  "zim": {
    ...
    specialInit: async () => {
      ...
      if (!await exists(initPath)) {
        ...
        await runCommand("timeout 30 zsh -c 'export ZIM_HOME=~/.zim; source ${ZIM_HOME}/zimfw.zsh init -q' 2>&1");
      }
    },
    postInstallCommand: "zsh -c 'export ZIM_HOME=~/.zim; source ${ZIM_HOME}/zimfw.zsh install' 2>&1",
  },
  
  "znap": {
    name: "znap",
    cacheCleanCommand: 'rm -rf ~/.cache/znap 2>/dev/null || true; find ~/Git -maxdepth 1 -name "zsh-*" ! -name "zsh-snap" -exec rm -rf {} + 2>/dev/null || true',
    configFiles: [
  ...
  "zinit": {
    name: "zinit",
    cacheCleanCommand: 'find ~/.local/share/zinit -mindepth 1 -maxdepth 1 ! -name "zinit.git" -exec rm -rf {} + 2>/dev/null || true',
    configFiles: [
  ...
  "zplug": {
    name: "zplug",
    cacheCleanCommand: 'if [[ -d ~/.zplug/.git ]]; then git -C ~/.zplug clean -dffx; else rm -rf ~/.zplug/repos ~/.zplug/cache; fi 2>/dev/null || true',
    configFiles: [
  ...
  "antibody": {
    ...
    configFiles: [
      ...
      { path: "~/.antibody_plugins.txt", template: "antibody.plugins.txt", isPluginList: true }
    ],
    generatePluginLoad: (plugin) => `antibody bundle ${plugin}`,
    preInstallCommand: "antibody bundle < ~/.antibody_plugins.txt > ~/.antibody_plugins.sh",
  },
  
  "antidote": {
    name: "antidote",
    cacheCleanCommand: "rm -rf ~/.cache/antidote ~/.zsh_plugins.zsh 2>/dev/null || true",
    configFiles: [
      { path: "~/.zshrc", template: "antidote.zshrc" },
      { path: "~/.zsh_plugins.txt", template: "antidote.zsh_plugins.txt", isPluginList: true }
    ],
  ...
  "sheldon": {
    name: "sheldon",
    cacheCleanCommand: "rm -rf ~/.local/share/sheldon ~/.cache/sheldon 2>/dev/null || true",
    configFiles: [
      { path: "~/.config/sheldon/plugins.toml", template: "sheldon.plugins.toml" },
  ...
  "zgenom": {
    name: "zgenom",
    cacheCleanCommand: 'if [[ -d ~/.zgenom/.git ]]; then git -C ~/.zgenom clean -dffx; else rm -rf ~/.zgenom/sources ~/.zgenom/*.zsh ~/.zgenom/*.zwc; fi 2>/dev/null || true',
    configFiles: [
  ...
  "zcomet": {
    name: "zcomet",
    cacheCleanCommand: "rm -rf ~/.zcomet/downloads ~/.zcomet/repos 2>/dev/null || true",
    configFiles: [
