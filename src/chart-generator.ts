...
async function main() {
  try {
    ...
    // Save charts
    const installPath =
      `${DEFAULT_CONFIG.paths.results}/install-time-comparison-chart.svg`;
    const loadPath =
      `${DEFAULT_CONFIG.paths.results}/load-time-comparison-chart.svg`;
    ...
    logger.error("Failed to generate comparison charts", error as Error);
    Deno.exit(1);
  }
}
