...
export class ChartGenerator {
  ...
  generateScalabilityChart(data: BenchmarkData): string {
    ...
    // Find max values
    const maxLoadTime = Math.max(...data.results.map((r) => r.loadTime || 0)) *
      1.1;
  ...
  <text x="${
        width / 2
      }" y="30" text-anchor="middle" font-size="20" font-weight="bold" font-family="Arial, sans-serif">Load Time Scalability (0 to 25 plugins)</text>
  ...
  <text x="${x}" y="${
        height - margin.bottom + 20
      }" text-anchor="middle" font-size="12" font-family="Arial, sans-serif">${label}</text>`;
    });
  ...
  <text x="${width / 2}" y="${
      height - 20
    }" text-anchor="middle" font-size="14" font-family="Arial, sans-serif">Number of Plugins</text>`;
    ...
    sortedManagers.forEach((manager, index) => {
      ...
      if (points.length > 0) {
        const pathData = points.map((point) => {
          ...
          const y = chartHeight -
            (point.loadTime! / maxLoadTime) * chartHeight + margin.top;
          return `${x},${y.toFixed(2)}`;
        }).join(" ");

        svg += `
  <polyline points="${pathData}" fill="none" stroke="${
          getColor(index)
  ...
  <circle cx="${x}" cy="${y}" r="4" fill="${getColor(index)}" opacity="0.8"/>`;
    ...
    sortedManagers.forEach((manager, index) => {
      svg += `
  <rect x="${legendX}" y="${legendY}" width="15" height="15" fill="${
        getColor(index)
      }" opacity="0.8"/>
  <text x="${legendX + 20}" y="${
        legendY + 12
      }" font-size="12" font-family="Arial, sans-serif">${manager}</text>`;
      legendY += 20;
    });
